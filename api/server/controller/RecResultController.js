...
const recomResult = async (req, res,next) => {
  const { username, habitat, criteria } = req.body;
  ...
  try {
    ...
    for (let l = 0; l < final_score_habitat.length; l++) {
      final_score =
        parseFloat(final_score_habitat[l].hab_score) +
        final_score_criteria[l].crite_score;
      ...
      include: [
        {
          ...
          as: "pet_images",
          attributes: ["petimg_filename", "petimg_filesize", "petimg_filetype"],
        },
      ],
    });
