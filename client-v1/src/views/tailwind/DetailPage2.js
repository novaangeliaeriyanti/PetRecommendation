...
export default function DetailPage() {
  ...
  const onCheckTalent = (item) => (event) => {
    ...
    if (tampung[0] === true) {
      ...
      const filterdTalent = talentCheck.filter((criteria) => criteria !== item);

      //console.log(filterdTalent);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (talentCheck.length === 0) {
        ...
        for(let i=0;i<talentCheck.length;i++){
        ...
        const pet_sort = pet_recom_count.sort((a, b) => parseFloat(a.pet_score) + parseFloat(b.pet_score));

        if(pet_sort[0].pet_score);
        let pet_result = pet_sort[0];
        const pet_recommendation = allpet[0].filter((values) => values.pet_id === pet_result.pet_id);
              ...
              {(bootcamps[0] || []).map((criteria) => (
                <label class="relative flex w-56 mt-12 mb-10 bg-none items-center hover:scale-105">
                  <input
                    ...
                    name="crite_name"
                    id="crite_name"
                    onClick={onCheckTalent(criteria)}
                  />
